...
async function renderAlunos() {
  ...
  content.innerHTML = `
    ...
    <h1 style="margin-bottom:20px;">${alunos.length} alunos cadastrados</h1>
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; align-items:start;">
      <div class="card">
        ${corners()}
        <div class="mono muted" style="font-size:11px; letter-spacing:.06em; text-transform:uppercase; margin-bottom:12px;">Adicionar alunos</div>
        <div class="field">
          <label>Nomes (um por linha)</label>
          <textarea id="novos-alunos" placeholder="Ana Souza
Bruno Carvalho
Camila Ferreira"></textarea>
        </div>
        <div id="erro-alunos"></div>
        <button class="btn" id="btn-add-alunos">+ Adicionar à turma</button>
      </div>
      <div class="card">
        ${corners()}
        <div class="mono muted" style="font-size:11px; letter-spacing:.06em; text-transform:uppercase; margin-bottom:12px;">Turma atual</div>
        ${alunos.length === 0 ? `<div class="muted" style="font-size:13px;">Nenhum aluno cadastrado ainda.</div>` : ""}
        ${alunos.map((a, i) => `
  ...
  document.getElementById("btn-add-alunos").addEventListener("click", async () => {
    ...
    try {
      await api("/alunos", { method: "POST", body: JSON.stringify({ nomes }) });
      renderAlunos();
    } catch (e) {
      erroEl.innerHTML = `<div class="error-box">${e.message}</div>`;
    }
...
async function renderMontar() {
  ...
  if (alunos.length === 0) {
    content.innerHTML = `<div class="card muted" style="text-align:center; padding:36px; font-size:13px;">${corners()}
      Você ainda não cadastrou nenhum aluno. <a href="#" id="link-alunos" style="color:var(--teal); text-decoration:underline;">Cadastrar alunos</a> antes de criar uma prova.
    </div>`;
    document.getElementById("link-alunos").addEventListener("click", (e) => { e.preventDefault(); setView("alunos"); });
    return;
  }

  content.innerHTML = `
    <div class="eyebrow">NOVA PROVA-MESTRE</div>
    <h1 style="margin-bottom:20px;">Selecione questões e alunos</h1>
    <div style="display:grid; grid-template-columns:1.3fr 1fr; gap:16px; align-items:start;">
      ...
      <div class="card">
        ...
        <div class="field">
          ...
          <input id="titulo" value="Avaliação Bimestral" />
        </div>
        <div class="field">
          <label>Turma</label>
          <input id="turma" value="Turma A" />
        </div>
        <div class="field">
