...
alunoExamRouter.get("/:token", asyncHandler(async (req, res) => {
...
alunoExamRouter.post("/:token/responder", asyncHandler(async (req, res) => {
  const { token } = req.params;
...
alunoExamRouter.post("/:token/finalizar", asyncHandler(async (req, res) => {
  const { token } = req.params;
