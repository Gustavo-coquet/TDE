...
// POST /api/provas-mestre/:id/zerar-tentativas   body: { alunoId }
// Apaga TODAS as tentativas deste aluno neste TDE (respostas, notas e ajuste manual) e gera
// de novo a tentativa 1, como se o TDE tivesse acabado de ser publicado para ele. Serve pra
// desfazer uma prova finalizada por engano. A tentativa 1 nova sai com a mesma semente da
// original (hash do TDE + aluno), ou seja, os mesmos valores e a mesma ordem de antes.
provasRouter.post("/:id/zerar-tentativas", asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { alunoId } = req.body as { alunoId?: string };
  ...
  const existentes = await prisma.provaIndividual.count({ where: { provaMestreId: id, alunoId } });
  if (existentes === 0) return res.status(404).json({ erro: "Este aluno não tem prova gerada neste TDE." });
  ...
  // tudo ou nada: se a criacao falhar, as tentativas antigas NAO somem
  await prisma.$transaction([
    prisma.provaIndividualQuestao.deleteMany({ where: { provaIndividual: { provaMestreId: id, alunoId } } }),
    prisma.provaIndividual.deleteMany({ where: { provaMestreId: id, alunoId } }),
    prisma.provaIndividual.create({
      data: {
        provaMestreId: id,
        alunoId: aluno.id,
        tentativa: 1,
