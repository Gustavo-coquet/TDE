...
// POST /api/provas-mestre  { titulo, turmaId, prazoFinal, questaoIds: string[] }
// prazoFinal é opcional (ISO string) — se não informado, o TDE fica sem prazo.
provasRouter.post("/", asyncHandler(async (req, res) => {
  const { titulo, turmaId, prazoFinal, questaoIds } = req.body;
...
provasRouter.get("/:id/resultados", asyncHandler(async (req, res) => {
  ...
  const linhas = Array.from(porAluno.values()).map((tentativas) => {
    ...
    for (const t of finalizadas) {
      const nota = (t.acertos! / t.total!) * 10;
