...
// body: { tema, dificuldade, enunciado, variaveis: [{nome,min,max,decimais}], etapas: [{nome,formula,decimais,unidade,saida}] }
questoesRouter.post("/", asyncHandler(async (req, res) => {
  const { tema, dificuldade, enunciado, variaveis, etapas, imagem } = req.body;
