        ...
        ${provas.map((p, i) => `
          <div style="padding:10px 0; ${i>0?'border-top:1px solid var(--line-faint);':''}">
            ...
            <div class="row" style="margin-top:6px;">
              <span class="muted" style="font-size:11.5px;">${p.totalQuestoes} questões · ${p.totalAlunos} alunos${p.prazoFinal ? ` · prazo até ${new Date(p.prazoFinal).toLocaleDateString("pt-BR")}` : ""}</span>
...
function renderFormNovaQuestao(container) {
  ...
  function validar(q) {
    if (!q.tema || !q.enunciado || q.variaveis.length === 0 || q.etapas.length === 0) {
      return "Preencha tema, enunciado, ao menos uma variável e ao menos uma etapa.";
    }
  ...
  document.getElementById("btn-publicar").addEventListener("click", async (e) => {
    ...
    try {
      ...
      const provaMestre = await api("/provas-mestre", {
        method: "POST",
        body: JSON.stringify({ titulo, turmaId: state.turmaAtualId, prazoFinal, questaoIds }),
      });
...
async function renderResultados() {
  ...
  content.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="row" style="margin-bottom:6px;">
        <div class="mono muted" style="font-size:11px; letter-spacing:.06em; text-transform:uppercase;">Notas por aluno (${r.totalFinalizadas}/${r.totalAlunos} finalizaram)</div>
  ...
  document.getElementById("btn-exportar-excel").addEventListener("click", () => {
    const linhas = r.alunos.map((a) => ({
      ...
      "Situação": a.nota === null ? "Não finalizou" : (a.nota >= 6 ? "Aprovado" : "Reprovado"),
    }));
    const ws = XLSX.utils.json_to_sheet(linhas);
    ws["!cols"] = [{ wch: 32 }, { wch: 14 }, { wch: 8 }, { wch: 10 }, { wch: 14 }];
