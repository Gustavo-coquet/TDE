  ...
  content.innerHTML = `
    <div style="text-align:center; display:flex; flex-direction:column; gap:16px; align-items:center; padding-top:20px;">
      ...
      <p class="muted" style="font-size:13.5px;">
        Você é <b style="color:var(--ink);">${dados.alunoNome}</b>. Sua melhor nota até agora é <b style="color:var(--ink);">${dados.melhorNota.toFixed(1)}</b>,
        em ${dados.tentativasFeitas} tentativa${dados.tentativasFeitas > 1 ? "s" : ""}.
      </p>
      ${dados.podeTentarDeNovo ? `
        <p class="muted" style="font-size:13px;">Você ainda pode fazer uma segunda tentativa. Vale a <b style="color:var(--ink);">maior</b> das duas notas.</p>
  ...
  content.innerHTML = `
    <div style="text-align:center; display:flex; flex-direction:column; gap:18px; align-items:center; padding-top:10px;">
      ...
      <p class="muted" style="font-size:14px; line-height:1.7;">
        Você é <b style="color:var(--ink);">${p.alunoNome}</b> — Tentativa <b style="color:var(--ink);">${p.tentativa}</b> de no máximo 2.
        Esta prova contém <b style="color:var(--ink);">${p.questoes.length} questões</b>, equivalentes às de todos os colegas da turma —
        mesmos temas e mesma dificuldade, com valores individuais para cada aluno.
      </p>
      <div class="card" style="width:100%; text-align:left;">
        <div class="corner tl"></div><div class="corner tr"></div><div class="corner bl"></div><div class="corner br"></div>
        <div class="row" style="font-size:13px; padding:6px 0;"><span class="muted">Questões</span><span>${p.questoes.length} de múltipla escolha</span></div>
      ...
      <p class="muted" style="font-size:12px;">Sem tempo limite por sessão — responda com calma. Pode fechar e voltar depois, dentro do prazo.</p>
