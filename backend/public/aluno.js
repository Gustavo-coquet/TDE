...
async function iniciar() {
  if (!token) {
    content.innerHTML = `<div class="error-box">Nenhum token de prova informado na URL. Use o link individual gerado na área do professor.</div>`;
    return;
  }
  try {
...
function renderInstrucoes() {
  ...
  content.innerHTML = `
    <div style="text-align:center; display:flex; flex-direction:column; gap:18px; align-items:center; padding-top:10px;">
      <div class="seal">✓ PROVA-MESTRE · EQUIVALÊNCIA VERIFICADA</div>
      <h1>${p.tituloProva}</h1>
      ...
      <div class="card" style="width:100%; text-align:left;">
        ...
        <div class="row" style="font-size:13px; padding:6px 0; border-top:1px solid var(--line-faint);"><span class="muted">ID da prova</span><span class="mono" style="color:var(--teal);">${token.slice(0,8)}</span></div>
      </div>
      ...
      <div style="margin-top:18px;">
        ${q.alternativas.map((a) => `
          <div class="option ${state.respostas[q.id]===a.letra?'selected':''}" data-letra="${a.letra}">
            ...
            <span class="mono" style="font-size:14px;">${a.valor} ${q.unidade}</span>
          </div>
        `).join("")}
      </div>
    </div>
